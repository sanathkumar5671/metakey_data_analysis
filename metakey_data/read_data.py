...
1. unique_holders_percentage -> in this function the percentage of the number of unique holders is calculated 
   using the formula [(unique holders number/total number of unique holders)*100]. The output is in format -> [{name: Edition 1, value: 71%}]
...
class Metakeydata:
    ...
    def unique_holders_percentage(self:object) -> Dict[str,float]:
        ...
        unique_holders_list = {}
        # This condition is added for error handling
        if 'edition' in self.df.columns and 'Unique Holders' in self.df.columns:
            ...
            for key, value in unique_holders_list.items():
                ...
                # the the formula [(unique holders number/total number of unique holders)*100] is used 
